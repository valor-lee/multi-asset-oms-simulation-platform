...
Trade(CAPTURED)
    ...
    -> accounting posting
    -> PositionLedgerEntry, CashLedgerEntry, AverageCostEntry, RealizedPnlEntry(SELL)
    -> balance query
    -> current position / current cash / current average cost
    -> PnL query
...
## 5. Post-Settlement Accounting Posting

```http
POST /api/post-trade/trades/{tradeId}/accounting-postings
```

### Request

request body는 없다. accounting workflow에 반영할 trade는 path variable의 `tradeId`로 식별한다.
...
{
  "positionLedgerEntry": {
    "entryId": "00000000-0000-0000-0000-000000084001",
    "tradeId": "00000000-0000-0000-0000-000000083001",
    "portfolioId": "portfolio-1",
    "instrumentId": "005930",
    "side": "SELL",
    "quantityDelta": -4,
  ...
  "cashLedgerEntry": {
    "entryId": "00000000-0000-0000-0000-000000085001",
    "tradeId": "00000000-0000-0000-0000-000000083001",
    "portfolioId": "portfolio-1",
    "side": "SELL",
    "cashDelta": 219920,
  ...
  "averageCostEntry": {
    "entryId": "00000000-0000-0000-0000-000000086001",
    "tradeId": "00000000-0000-0000-0000-000000083001",
    "portfolioId": "portfolio-1",
    "instrumentId": "005930",
    "side": "SELL",
    "quantity": 4,
    "costDelta": -216000.0000000000,
    "positionQuantity": 6,
    "costBasis": 324000.0000000000,
    "averageCost": 54000.0000000000,
    "postedAt": "2026-06-23T00:00:00Z"
  },
  "realizedPnlEntry": {
    "entryId": "00000000-0000-0000-0000-000000087001",
    "tradeId": "00000000-0000-0000-0000-000000083001",
    "portfolioId": "portfolio-1",
    "instrumentId": "005930",
    "quantity": 4,
    "averageSellPrice": 55000,
    "averageCost": 54000.0000000000,
    "grossNotional": 220000,
    "feeAmount": 80,
    "taxAmount": 0,
    "realizedPnl": 3920.0000000000,
    "postedAt": "2026-06-23T00:00:00Z"
  }
}
```

## Accounting Posting 규칙

| Trade | 처리 |
| --- | --- |
| `SETTLED` BUY | position/cash ledger와 average cost 원장에 반영하고 `realizedPnlEntry`는 `null`로 응답 |
| `SETTLED` SELL | position/cash ledger, average cost 원장, realized PnL 원장에 함께 반영 |
| 이미 posting된 trade | 각 원장의 기존 entry를 반환 |
| 그 외 상태 | `409 Conflict` |

이 API는 운영 흐름에서 settlement 완료 후 호출하는 묶음 use case다. 개별 `ledger-postings`, `average-cost-postings`, `realized-pnl-postings` API는 테스트/시뮬레이션과 장애 복구용 경계로 유지한다.

실제 DB 영속화 단계에서는 이 workflow를 하나의 transaction으로 묶거나, ledger/PnL posting을 outbox 기반으로 재처리할 수 있게 설계하는 것이 좋다.
...
accounting posting 대상이 아니면 `409 Conflict`를 반환한다.

```json
{
  "message": "only SETTLED trades can be posted to accounting"
}
```

## 6. Current Position 조회
...
## 11. Current Average Cost 기준 Realized PnL Posting
...
## 14. Latest Market Price 기준 Unrealized PnL Snapshot 조회
...
## 15. Current Average Cost + Latest Market Price 기준 Unrealized PnL Snapshot 조회
