...
기존 ACK, reject, fill API가 broker/exchange 이벤트를 직접 입력하는 경계라면, 이 API는 MVP execution simulator가 broker/exchange 역할을 대신해 지연, reject 확률, ACK, 체결 가능 여부, 체결 가능 수량, 체결 가격을 한 번에 계산하는 운영형 경계다.
...
| `rejectRate` | no | `SENT` 주문이 ACK 전에 broker/exchange에서 거절될 확률. 생략하면 `0`, 허용 범위는 `0 <= rate <= 1` |
| `availableQuantity` | no | 이번 시뮬레이션에서 시장에 있다고 가정하는 체결 가능 수량. 생략하면 `fillQuantity` 전체를 체결 가능 수량으로 본다. 허용 범위는 `0 <= quantity` |

같은 `simulationId`, `orderId`, `fillQuantity`, `slippageRate`, `rejectRate`, `availableQuantity`로 재요청하면 저장된 결과를 반환한다. 같은 `simulationId`를 다른 주문이나 다른 payload에 사용하면 `409 Conflict`를 반환한다.
...
### Liquidity Constrained Partial Fill Response

`fillQuantity`는 이번 요청에서 체결을 시도하는 수량이고, `availableQuantity`는 시장에 있다고 가정하는 체결 가능 수량이다. `availableQuantity`가 `fillQuantity`보다 작으면 실제 체결 수량은 `availableQuantity`로 제한된다.

```json
{
  "simulationId": "00000000-0000-0000-0000-000000073004",
  "orderId": "00000000-0000-0000-0000-000000072004",
  "simulationStatus": "FILLED",
  "referencePrice": 55000,
  "fillPrice": 55000,
  "requestedFillQuantity": 8,
  "executedFillQuantity": 3,
  "availableQuantity": 3,
  "slippageRate": 0,
  "rejectRate": 0,
  "delayMillis": 80,
...
`availableQuantity=0`이면 가격 조건을 만족하더라도 체결 수량이 없으므로 `NOT_FILLED`로 저장한다. `SENT` 주문이었다면 ACK까지는 반영되고 fill execution은 생성되지 않는다.

### Broker Reject Response

`SENT` 주문에서 `rejectRate`가 확률 조건에 걸리면 ACK와 가격 조회 전에 broker/exchange reject 이벤트를 반영한다. 이 경우 체결 시도가 없으므로 `referencePrice`와 `fillPrice`는 `null`이다.

```json
{
  "simulationId": "00000000-0000-0000-0000-000000073003",
  "orderId": "00000000-0000-0000-0000-000000072003",
...
`rejectRate`는 broker/exchange가 주문을 아예 받아들이지 않는 상황을 재현하기 위한 시뮬레이션 정책이다. 예를 들어 거래 정지, 브로커 내부 제한, 라우팅 실패 같은 외부 venue 응답을 MVP에서 확률값으로 단순화한 것이다.

## Simulation 상태 규칙

| Order status | 처리 |
| --- | --- |
| `SENT` | 20~200ms 지연 후 reject 확률을 먼저 평가하고, reject되지 않으면 ACK를 반영한 뒤 가격/수량 기준으로 체결 평가 |
| `ACKED` | 지연 후 가격/수량 기준으로 체결 평가 |
| `PARTIALLY_FILLED` | 지연 후 남은 수량에 추가 체결 평가 |
| 그 외 상태 | `409 Conflict` |

`ACKED`, `PARTIALLY_FILLED` 주문은 이미 broker/exchange가 주문을 접수한 뒤의 상태이므로 `rejectRate`를 주더라도 broker reject를 다시 평가하지 않는다. 최신 시장 가격이 없으면 `404 Not Found`를 반환한다. 요청 수량이 잔여 수량을 초과하면 기존 fill 규칙에 따라 `409 Conflict`를 반환한다.
...
기존 ACK/reject/fill API는 외부 이벤트 직접 입력, 테스트, 장애 복구 경계로 계속 유지한다.
