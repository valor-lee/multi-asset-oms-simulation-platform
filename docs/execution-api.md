...
기존 ACK, reject, fill API가 broker/exchange 이벤트를 직접 입력하는 경계라면, 이 API는 MVP execution simulator가 broker/exchange 역할을 대신해 지연, reject 확률, ACK, 체결 가능 여부, 체결 가격을 한 번에 계산하는 운영형 경계다.
...
같은 `simulationId`, `orderId`, `fillQuantity`, `slippageRate`, `rejectRate`로 재요청하면 저장된 결과를 반환한다. 같은 `simulationId`를 다른 주문이나 다른 payload에 사용하면 `409 Conflict`를 반환한다.
...
| Order status | 처리 |
| --- | --- |
| `SENT` | 20~200ms 지연 후 reject 확률을 먼저 평가하고, reject되지 않으면 ACK를 반영한 뒤 체결 평가 |
| `ACKED` | 지연 후 체결 평가 |
